...
class ArchitectRole(RolePlugin):
    ...
    async def review(
        ...
        Returns:
            RoleResult with architecture findings
        """
        started_at = datetime.now()

        # The actual review is performed by the AI client (Goose)
        # using the recipe generated by get_recipe_template()
        # This method returns a placeholder result

        return RoleResult(
            role=self.name,
            status=RoleStatus.PENDING,
            success=False,
            output="Architecture review pending - run with AI client",
            issues=[],
            suggestions=[
                "Run the architect recipe to perform full architecture review",
                "Review specs/arch.md for current architecture",
                "Ensure component diagram is up to date",
            ],
